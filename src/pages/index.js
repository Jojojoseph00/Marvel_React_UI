...
const IndexPage = () => {
  ...
  const ThanosGetter = ({ children }) => {
    ...
    useEffect(() => {
      axios
        .get(
          `https://gateway.marvel.com:443/v1/public/comics?title=${selectedTitle}&orderBy=onsaleDate&limit=${limit}&ts=${timestamp}&apikey=${api_key}&hash=${hash_string}`
        )
        .then(res => {
